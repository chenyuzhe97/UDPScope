...
void PlotWidget::onFrameAdvanced(uint64_t widx) {
    // 简单的 FPS 估计（指数平滑）
    double dt = fpsTimer_.elapsed() / 1000.0;
    if (dt >= 0.3) {
        uint64_t delta = (widx > lastFrameCount_) ? (widx - lastFrameCount_) : 0;
        double inst = delta / std::max(1e-6, dt);
        measuredFps_ = 0.9 * measuredFps_ + 0.1 * inst;
        lastFrameCount_ = widx; fpsTimer_.restart();
    }
    widx_ = widx;
    update();
}
...
void PlotWidget::paintGL() {
    ...
    if (!ring_ || widx_==0) { p.setPen(Qt::white); p.drawText(rect(), Qt::AlignCenter, "waiting..."); return; }
    ...
    for (int i=bins_-1;i>=0;--i) shaded.lineTo(X(env.x[i]), Y(env.ymin[i]));
    shaded.closeSubpath();
    ...
    smooth_ema(mean, dt_bin, 8.0); // 先给个默认 EMA，可做 UI 参数

    QPainterPath m; m.moveTo(X(env.x.front()), Y(mean.front()));
    for (int i=1;i<bins_;++i) m.lineTo(X(env.x[i]), Y(mean[i]));

    QPen pen(Qt::white); pen.setWidth(1); p.setPen(pen);
    p.drawPath(m);

    // 边框与刻度（简化）
    p.setPen(QColor(180,180,180)); p.drawRect(R);
